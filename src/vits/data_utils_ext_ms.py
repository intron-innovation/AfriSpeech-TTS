...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams):
        ...
        speaker_emb_path = getattr(hparams, "speaker_emb_path")
        if speaker_emb_path is not None:
            self.speaker_emb_dict= load_speaker_emb(speaker_emb_path)
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        spk_emb = self.speaker_emb_dict.get(Path(audiopath).stem)
        spk_emb = torch.tensor(spk_emb)
        return (text, spec, wav, spk_emb)
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            spk_emb = row[3]
            spk_emb_arr.append(spk_emb.reshape(1, -1))
            
        sid = torch.cat(spk_emb_arr, dim=0)
